...
if __name__ == "__main__":
    ...
    for split in [train, val, test]:
        ...
        split = add_protocol_type_features(split)
        split = add_treatment_label_encoding(split)
        split = add_log_type_features(split)
        split = add_bytes_transferred_scaling(split, mean, std)
        split = add_user_agent_features(split)
        split = add_path_structure_features(split)
        # Select only features and target, dropping raw columns
        split = split.select(FEATURE_COLUMNS + [TARGET_COLUMN])
